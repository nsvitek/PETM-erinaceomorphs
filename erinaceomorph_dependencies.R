...
# source("function_bootstrap.R")
# source("disparity_pdp.R")
source("rotate3dShape.R")
source("BHB-erinaceomorph/binsBHB.R")
...
source("plyClip_Update.R") #temporary update from P.E.Morse
...
plot_linear<-function(dataset,variable,x_label,colors=bin.col,o18curve=TRUE){
  if(o18curve==TRUE){
    ...
    ggplot(data=dataset, aes_string(y="meter_level",x=variable,group="meter_level"))+
      ...
      geom_point(aes(fill=bin2,shape=bin2),alpha=0.95,size=4) +
